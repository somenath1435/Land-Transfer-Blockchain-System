...
class RegOffDetails extends Component {
  ...
  async componentDidMount() {
    try{
      ...
      const list= await factory.methods.registryofficeraddress(0).call();
      console.log(list);
      console.log(addr);
  ...
  renderCampaigns() {
    ...
    const items = [
      ...
      {
        header: "Registry Officer Government ID",
  ...
  render() {
    return (
      <Layout>
        <div>
          <h3>Registry Officer Details for address {this.props.address}</h3>
          ...
          <br/><br/>
          <Link route={`/regoff/${this.props.address}/pendingrequest`}>
            <a>
